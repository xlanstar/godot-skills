...
- Prefer static types and typed containers to enable optimized instructions and avoid `Variant` dispatch.
- Prefer `@onready`, `preload()`, early returns, signals, groups, cached nodes, and `StringName` literals (`&"name"`) for repeated identifiers.
- Avoid per-frame allocations. Use packed arrays for bulk buffers and integer vectors for grid data when their semantics fit.
- Use static methods when no instance state is needed.

## Callbacks

Match the class reference exactly; never change a virtual method's parameter count, names, types, or return type. Processing for `_process`, `_physics_process`, `_input`, `_shortcut_input`, `_unhandled_key_input`, and `_unhandled_input` is enabled automatically when the method is overridden; the matching `set_process_*()` call only toggles it afterwards.
...
- `_init()` runs before the node is in a tree, so `@onready` variables and `get_node()` are unavailable there. A parameterized `_init()` forces every `.new()` call to pass those arguments.
...
- After resumption, recheck referenced objects with `is_instance_valid()`, tree membership with `is_inside_tree()`, and the state or generation that started the work.
- Prevent duplicate concurrent runs with a busy flag or generation ID when re-entry is possible.
